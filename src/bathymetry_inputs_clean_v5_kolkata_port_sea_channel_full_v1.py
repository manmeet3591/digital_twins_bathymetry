# -*- coding: utf-8 -*-
"""bathymetry_inputs_clean_v5_kolkata_port_sea_channel_full_v1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1emRpI8cy71SX1TlC72wmoHSho8BR37mD
"""
...
!ls drive/MyDrive/Bathy/Survey_Data/Kolkata_port_sea_channel/
#!ls drive/MyDrive/Bathy/Survey_Data/Dhurwa_full/DHURWA_DAM_1M_MSL_DEPTH_1_2022.xyz

import pandas as pd
df = pd.read_csv('drive/MyDrive/Bathy/Survey_Data/Kolkata_port_sea_channel/Eastern_Channe_January_2020.xyz', header=None)
df = pd.concat([df, pd.read_csv('drive/MyDrive/Bathy/Survey_Data/Kolkata_port_sea_channel/Western_Channel_Dec_2019.XYZ', header=None)]).sort_index()
df

# df = pd.read_csv('drive/MyDrive/Bathy/Survey_Data/Dhurwa_full/DHURWA_DAM_1M_MSL_DEPTH_1_2022.xyz', header=None)
# df
...
#epsg, hemisphere, north_south = get_epsg(dam_name='Hirakud', country='India')
...
df['bathy'] = df.iloc[:,2] #np.max(df.iloc[:,2].values) - df.iloc[:,2].values
...
!ls
!mv figure.png drive/MyDrive/Bathy/Survey_Data/Kolkata_port_sea_channel/
...
ds_sentinel = dataset.wx.to_xarray(region=aoi.bounds(), scale=100)#,  crs='EPSG:32645')
...
dataset = dataset.set('system:time_start', 0)
ds_landsat = dataset.wx.to_xarray(region=aoi.bounds(), scale=100)#,  crs='EPSG:32645')

ds_landsat.SR_B5.mean(dim='time').plot(cmap='coolwarm')
...
spatial_extent = [coords[0][0], coords[0][1], coords[-3][0],coords[-3][1]]

# Given as longitude, latitude coordinate pairs
date_range = ['2019-12-01','2020-01-30']
...
file = h5py.File('./download/processed_ATL13_20191225001313_13610501_006_01.h5', 'r')
# List all groups
print("Keys: %s" % file.keys())

group_key = list(file.keys())[9]

# Get the group
group = file[group_key]
# Now you can access keys of the group, which are the variables
print("Group Keys: %s" % group.keys())

#lat, lon, water_depth = 2,3, -1

variable_key = list(group.keys())[3]
# Get the variable
variable = group[variable_key]
# Now you can access data of the variable
data = variable[:]
print(data)
...
#         latvar = f['/gt3r/bottom_lat']
#         latitude = latvar[:]

#         lonvar = f['/gt3r/bottom_lon']
#         longitude = lonvar[:]

#         dset_name = '/gt3r/segment_geoid'
#         datavar = f[dset_name]
#         geoid = datavar[:]

#         datavar = f['/gt3r/ht_ortho']
#         ht_ortho = datavar[:]

#         datavar = f['/gt3r/water_depth']
#         ht_water_surf = datavar[:]


#         datavar = f['/gt3r/err_ht_water_surf']
...
date_range = ['2019-12-01','2020-02-28']

gedi = ee.ImageCollection('LARSE/GEDI/GEDI02_A_002_MONTHLY').map(qualityMask).filterBounds(aoi).filterDate('2019-12-01','2019-12-30' )
...
fig = px.scatter_mapbox(df_gedi, lat="y", lon="x", color="rh98",
                  color_continuous_scale=px.colors.cyclical.IceFire, size_max=5, zoom=8,
                  mapbox_style="carto-positron")
fig.show()
...
dict_ = {'Time': dates_list[:-1], 'Sedimentation':sedimentation_[:-1], 'Precipitation': precipitation_}
...
water_collection = collection.select('water').filterDate('2019-12-01', '2020-01-30').filterBounds(aoi)
ds_dyn_water = water_collection.wx.to_xarray(region=aoi.bounds(), scale=1000)

ds_dyn_water

water = ds_dyn_water.water.mean(dim='time').values[:,:].copy()
water[:,:] = 0.0
bool_ = ds_dyn_water.water.mean(dim='time').values[:,:]>0.2
water[bool_] = 1.0
...
df_training.to_csv('kolkata_port_sea_channel_training_sentinel_landsat.csv')

!mv kolkata_port_sea_channel_training_sentinel_landsat.csv /content/drive/MyDrive/Bathy/Training

df_training = pd.read_csv('/content/drive/MyDrive/Bathy/Training/kolkata_port_sea_channel_training_sentinel_landsat.csv')
...
ds_landsat_ = ds_landsat.mean(dim='time').interp(y=ds_sentinel.y.values).interp(x=ds_sentinel.x.values)

ds_ndwi_ = ds_ndwi.mean(dim='time').interp(y=ds_sentinel.y.values).interp(x=ds_sentinel.x.values)

ds_ndwi

ds_landsat_

ds_sentinel.to_netcdf('sentinel_kolkata_port_sea_channel_test_input.nc')
ds_landsat_.to_netcdf('landsat_kolkata_port_sea_channel_test_input.nc')
ds_ndwi_.to_netcdf('ndwi_kolkata_port_sea_channel_test_input.nc')

!ls

!mv sentinel_kolkata_port_sea_channel_test_input.nc landsat_kolkata_port_sea_channel_test_input.nc ndwi_kolkata_port_sea_channel_test_input.nc /content/drive/MyDrive/Bathy/Training

import xarray as xr
df_training = pd.read_csv('/content/drive/MyDrive/Bathy/Training/kolkata_port_sea_channel_training_sentinel_landsat.csv')
ds_sentinel = xr.open_dataset('/content/drive/MyDrive/Bathy/Training/sentinel_kolkata_port_sea_channel_test_input.nc')
ds_landsat_ = xr.open_dataset('/content/drive/MyDrive/Bathy/Training/landsat_kolkata_port_sea_channel_test_input.nc')
ds_ndwi_ = xr.open_dataset('/content/drive/MyDrive/Bathy/Training/ndwi_kolkata_port_sea_channel_test_input.nc')
...
fig = px.scatter_mapbox(df_test__, lat="y", lon="x", color="y_test_pred",
                  color_continuous_scale=px.colors.sequential.PuBu, size_max=5, zoom=8,
                  mapbox_style="carto-positron")
...
!mv figure_prediction.png figure.png drive/MyDrive/Bathy/Survey_Data/Kolkata_port_sea_channel/.
